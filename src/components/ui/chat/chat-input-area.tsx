...
interface ChatInputAreaProps {
  ...
  handleSubmit: (
    e: React.FormEvent<HTMLFormElement> | React.KeyboardEvent<HTMLTextAreaElement>, // Updated to include KeyboardEvent
...
function useAutoResizeTextarea({
  ...
  const adjustHeight = useCallback(
    (reset?: boolean) => {
      ...
      textarea.style.height = `${minHeight}px`; // Reset height first to correctly calculate scrollHeight
...
export function ChatInputArea({
  ...
  const availableModels = appContext?.availableModels ?? [];
  const selectedModel = appContext?.selectedModel ?? "claude-opus-4-20250514";
  const setSelectedModel = appContext?.setSelectedModel ?? (() => {});

  // Fallback to hardcoded models if availableModels is empty
  ...
  const AI_MODELS = availableModels.length > 0 ? availableModels : fallbackModels;
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if (e.key === "Enter" && !e.shiftKey && input.trim()) {
      e.preventDefault();
      handleSubmit(e); // Pass the keyboard event directly
  ...
  return (
    <div className={cn("w-3xl py-2", className)}>
